...
## UI 对话显示优化 — 2026-04-06

### 需求 1: SubtaskBlock 折叠只显示最新 child tool
- **Before**: 折叠时显示全部 child tools 列表
- **After**: 折叠时只显示最新一条 child tool + "+N more" 提示；展开后显示全部 child tools + prompt + result

### 需求 2: 代码块一键复制
- 代码块右上角添加复制图标按钮
- 默认透明，hover `<pre>` 时显示
- 点击后复制代码内容，图标切换为 ✓（1.5 秒后恢复）

### 文件改动
- [x] `frontend/src/components/ToolBlock.tsx`：SubtaskBlock 折叠/展开逻辑重构
- [x] `frontend/src/components/MarkdownRenderer.tsx`：新增 CodeBlock 组件（自定义 `pre` 渲染器 + 复制按钮）
- [x] `frontend/src/index.css`：`pre` 添加 `position: relative`，新增 `.code-copy-btn` 样式
